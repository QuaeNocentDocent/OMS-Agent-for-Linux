  ...
  class DscMonitoringInput < Input
    ...
    def run_check_install
      until @finished_check_install
        ...
        # store dsc installation status in the cache
        @dsc_cache.transaction {
          @dsc_cache[:install] = install_status 
          @dsc_cache.commit
        }
    ...
    def run_check_status
      ...
      # fetch value of dsc installation status from the cache
      # if key does not exist, assigns the string as value
      stored_install_status = @dsc_cache.transaction {
        @dsc_cache.fetch(:install, "DSC installation test status not stored yet")
      }

      if stored_install_status == 0 
